...
import cls from './styles/scss/loader.scss'
...
class Loader extends Component {
    ...
    render() {
        ...
        // attribute props
        const { title, desc, status, size, style } = this.props
        ...
        return (
            <div style={styles} className={cls['progress__wrapper']}>                
                <div className={cls['progress__holder']}>
                    {this.renderTiles()}
                </div>
                <div className={`${cls['progress__title']} ${(!title) && cls['progress_hide']}`}>{title}</div>
                <div className={`${cls['progress__desc']} ${(!title) && cls['progress_hide']} tc ${this.handleStatus(status)}`}>{desc}</div>
            </div>
        )
    }
}
